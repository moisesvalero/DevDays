...
const DAYS = [
	{
		...
		hero: 'Svelte = UI por componentes',
		sub: 'Padre/hijo: datos bajan, eventos suben.',
		labs: ['Árbol de componentes', 'Props down', 'Events up'],
		specs: ['Svelte', 'props', 'on:click']
	},
	{
		n: 16,
		id: 'dia-16-svelte-archivo',
		hero: 'Anatomía del .svelte',
		sub: 'script, markup y style.',
		labs: ['Bloque script', 'Markup', 'Style scoped'],
		specs: ['script', 'HTML', 'style']
	},
	{
		n: 17,
		id: 'dia-17-state',
		hero: '$state — estado mutable',
		sub: 'Carrito y contador en $state.',
		labs: ['Contador', 'Toggle UI', 'Formulario'],
		specs: ['$state', '$state bool', '$state string']
	},
	{
		n: 18,
		id: 'dia-18-derived',
		hero: '$derived y $effect',
		sub: 'IVA derivado y efectos.',
		labs: ['Precio + IVA', 'Stock bajo', 'Log simulado'],
		specs: ['$derived', '$derived bool', '$effect']
	},
	{
		n: 19,
		id: 'dia-19-props',
		...
		labs: ['Props obligatorias', 'Default props', 'Props tipadas'],
		specs: ['$props', 'default', 'types']
	},
	{
		n: 20,
		id: 'dia-20-eventos',
		hero: 'Eventos y bind:',
		sub: 'onclick y two-way.',
		labs: ['onclick', 'bind:value', 'bind:checked'],
		specs: ['onclick', 'bind:value', 'bind:checked']
	},
	{
		n: 22,
		id: 'dia-22-rutas',
		hero: 'Rutas SvelteKit',
		sub: 'Mapa de navegación.',
		labs: ['Navegación', 'Link activo', '404'],
		specs: ['nav', 'active', '404']
	},
	{
		n: 23,
		id: 'dia-23-layouts',
		hero: 'Layouts y slots',
		sub: 'Shell de app.',
		labs: ['Shell', 'Slot principal', 'Layout anidado'],
		specs: ['layout', 'slot', 'nested']
	},
	{
		n: 24,
		id: 'dia-24-dinamicas',
		...
		sub: 'Lista a detalle.',
		labs: ['Lista → detalle', 'params.slug', '404 producto'],
		specs: ['list', 'slug', '404']
	},
	{
		n: 25,
		id: 'dia-25-load',
		hero: 'load()',
		sub: 'Datos antes de pintar.',
		labs: ['load function', 'Skeleton', '$props data'],
		specs: ['load', 'skeleton', 'data']
	},
	{
		n: 26,
		id: 'dia-26-forms',
		hero: 'Form actions',
		sub: 'POST al servidor.',
		labs: ['POST action', 'Validación', 'Progressive'],
		specs: ['POST', 'valid', 'enhance']
	},
	{
		n: 27,
		id: 'dia-27-supabase',
		hero: 'Supabase',
		sub: 'Auth y datos.',
		labs: ['Login', 'Query', 'RLS'],
		specs: ['auth', 'query', 'RLS']
	},
	{
		n: 29,
		id: 'dia-29-typescript',
		hero: 'TypeScript',
		sub: 'Tipos en producto.',
		labs: ['Interface Producto', 'Props tipadas', 'Union types'],
		specs: ['interface', 'props', 'union']
	},
	{
		n: 30,
		id: 'dia-30-env',
		hero: 'Variables de entorno',
		sub: 'Públicas y secretas.',
		labs: ['Variables públicas', 'Secrets servidor', 'hooks.server'],
		specs: ['PUBLIC', 'SECRET', 'hooks']
	},
	{
		n: 31,
		id: 'dia-31-deploy',
		hero: 'Deploy Vercel',
		sub: 'Pipeline a producción.',
		labs: ['Pipeline', 'Preview', 'Env prod'],
		specs: ['CI', 'preview', 'prod']
	},
	{
		n: 32,
		id: 'dia-32-stack-story',
		hero: 'Pitch del stack',
		sub: 'Historia de 2 min.',
		labs: ['Problema de negocio', 'Stack elegido', 'Demo en vivo'],
		specs: ['problem', 'stack', 'demo']
	},
	{
		n: 33,
		...
		labs: ['Situación', 'Acción', 'Resultado'],
		specs: ['S', 'A', 'R']
	},
	{
		n: 34,
		...
		sub: 'Guiar la demo.',
		labs: ['Hook (15s)', 'Demo guiada', 'Next steps'],
		specs: ['hook', 'demo', 'next']
	}
];

function labComponent(_day, idx, titulo, spec) {
...
function visualLesson(day, cfg) {
	const imports = cfg.labs
		.map((_, i) => `import Lab${i + 1} from './Lab${i + 1}.svelte';`)
		.join('\n  ');
	const sections = cfg.labs
		.map(
			(t, i) =>
				`  <LabSection numero={${i + 1}} titulo="${t}" spec="${cfg.specs[i]}" anchorId={seccionAnchorId(dia, ${i})}><Lab${i + 1} /></LabSection>`
		)
		.join('\n');
	return `<script lang="ts">
