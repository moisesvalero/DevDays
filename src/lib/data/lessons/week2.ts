...
export const week2: Leccion[] = [
	{
		...
		objetivo:
			'Modelar entidades (usuario, producto, pedido) con objetos literales y acceder a sus propiedades.',
		contenido: contenidoLab('dia-8-objetos', 'Laboratorio: ficha de producto como objeto.', [
			'Card de producto',
		...
		ejercicios: [
			ej(
				...
				'Objeto producto',
				{
					...
					notas:
						'Siga los pasos de la sección «Objeto literal» (`producto.nombre` y `console.log`).'
				},
	...
	},
	{
		...
		objetivo:
			'Elegir entre for, for...of y map según si necesitas índice, iteración simple o transformación de lista.',
		contenido: contenidoLab('dia-9-bucles', 'Laboratorio: tablas y chips desde arrays.', [
			'Generar filas',
			'Chips de categorías',
			'includes en carrito'
		]),
		ejercicios: [
			...
			ej(
				...
				'Iterar con for...of',
				{
					...
					notas:
						'Véase la sección «Bucle for-of»: patrón `for (const fruta of frutas) { console.log(fruta); }`.'
				},
	...
	},
	{
		...
		objetivo:
			'Entender lexical scope y closures: funciones que conservan variables del entorno donde se crearon.',
		contenido: contenidoLab('dia-10-closures', 'Laboratorio: closures y factory de descuento.', [
			'Contador privado',
			'Factory descuento',
			'Precio final'
		]),
		ejercicios: [
			...
			ej(
				...
				'this vs runes',
				{
					...
					notas:
						'Consulte la sección «this (referencia breve)»; no hace falta código ejecutable más allá del template string.'
				},
	...
	},
	{
		...
		objetivo:
			'Representar operaciones asíncronas con Promise, then y catch antes de pasar a async/await.',
		contenido: contenidoLab('dia-11-promesas', 'Laboratorio: estados de Promise en checkout.', [
			'Simular carga',
			'Timeout',
			'Encadenar'
		]),
		ejercicios: [
			...
			ej(
				...
				'Resolver y catch',
				{
					...
					seccionRef: 'Timeout',
					notas:
						'Copie los patrones de la sección «then / catch» (`resolve` + `then` o `reject` + `catch`).'
				},
	...
	},
	{
		...
		objetivo:
			'Escribir flujo asíncrono lineal con async/await, try/catch y fetch para consumir APIs REST.',
		contenido: contenidoLab('dia-12-async', 'Laboratorio: async/await y estados de fetch.', [
			'Cargar catálogo',
			'Skeleton',
			'Retry'
		]),
		ejercicios: [
			...
			ej(
				...
				'try catch',
				{
					...
					seccionRef: 'Skeleton',
					notas:
						'Repita el patrón de la sección «try / catch» con `console.log("error")` en el bloque catch.'
				},
				['try y catch'],
			...
			ej(
				...
				'fetch público',
				{
					...
					seccionRef: 'Retry',
					notas:
						'Siga «fetch + JSON»: `await fetch`, luego `await res.json()` y `console.log(data.title)`.'
				},
	...
	},
	{
		...
		objetivo:
			'Conocer el DOM del navegador, eventos y almacenamiento local; Svelte abstrae la mayor parte del DOM manual.',
		contenido: contenidoLab('dia-13-dom', 'Laboratorio: eventos y localStorage simulado.', [
			'Tema claro/oscuro',
			'onclick',
			'Carrito guardado'
		]),
	...
	},
	{
		...
		objetivo:
			'Demostrar objetos, iteración, closures, promesas/async y APIs del navegador (sin exigir sintaxis perfecta).',
