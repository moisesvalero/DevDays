<script lang="ts">
	...
	const salidaTrimmed = $derived(enunciado.salidaEsperada.trim());
	const salidaEsCodigo = $derived(
		enunciado.salidaEsperada.includes('\n') ||
			'0123456789[{"\'`truefals-'.includes(salidaTrimmed[0] ?? '')
	);
...
<section class="space-y-4 text-sm leading-relaxed text-on-surface">
	...
	<section>
		...
		{:else}
			<pre
				class="overflow-x-auto rounded-lg border border-outline-variant bg-surface-container p-3 font-mono text-sm">{enunciado.salidaEsperada.trim()}</pre>
		{/if}
	</section>
