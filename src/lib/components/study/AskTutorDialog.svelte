<script lang="ts">
	...
	async function enviar() {
		...
		try {
			...
			mensajes = [...mensajes, { role: 'model', text: json.respuesta || 'Sin respuesta.' }];
		} catch {
			mensajes = [
				...mensajes,
	...
	<DialogContent class="flex max-h-[80vh] flex-col gap-0 p-0 sm:max-w-2xl">
		<DialogHeader class="border-b border-outline-variant p-5">
			...
			<DialogDescription class="text-on-surface-variant">
				Día {dia} · Ejercicio {ejercicio}. Pregunta lo que necesites. El tutor no aprueba ejercicios
				desde aquí, solo aclara dudas.
			</DialogDescription>
		</DialogHeader>
