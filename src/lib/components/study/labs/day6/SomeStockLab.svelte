...
<InteractiveLabLayout hint="some detecta si algún SKU está a 0 — la alerta global se enciende.">
  {#snippet preview()}
    <label class="mb-4 block max-w-xs space-y-2">
      ...
      <input type="range" min="0" max="3" bind:value={values.cero} class="w-full accent-destructive" />
    </label>
    ...
    {#if hayAgotado}
      <div class="mt-6 flex items-center gap-3 rounded-xl border border-destructive/40 bg-destructive/10 p-4">
