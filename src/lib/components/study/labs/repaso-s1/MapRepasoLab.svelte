...
<InteractiveLabLayout
	hint="Repaso semana 1 · Reto 3: map sobre [1,2,3] y console.log del resultado."
>
	{#snippet preview()}
		<div class="space-y-6">
			<div class="flex flex-wrap items-center gap-3">
				...
				<span class="font-mono text-xl font-bold text-primary"
					><LiveValue value={values.mult} /></span
				>
			</div>
			<div class="rounded-xl border border-primary/30 bg-primary/5 p-6">
				...
				<div class="mt-3 flex flex-wrap gap-2">
					{#each doblado as n (n)}
						<span
							class="rounded-lg bg-primary/20 px-4 py-2 font-mono text-lg font-bold text-primary"
							>{n}</span
						>
					{/each}
				</div>
