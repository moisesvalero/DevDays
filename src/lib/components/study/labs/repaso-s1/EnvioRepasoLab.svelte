...
<InteractiveLabLayout hint="Repaso semana 1 · Reto 1: replica este mensaje en consola con template literals.">
  {#snippet preview()}
    <div class="grid gap-6 lg:grid-cols-2">
      <div class="rounded-xl border border-outline-variant/40 bg-surface-container-low p-6 space-y-4">
  ...
  {#snippet spec()}
    <LiveSpecPanel
      ...
      lines={[
        ...
        { kind: 'let', key: 'nombre' },
        { kind: 'let', key: 'ciudad' },
        { kind: 'derived', name: 'mensaje', expr: '`Envío para ${nombre} hacia ${ciudad}`', resultKey: 'envio' }
      ]}
    />
  {/snippet}
