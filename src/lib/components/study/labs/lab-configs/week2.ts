...
export const day9: DayLabConfig = {
  ...
  labs: [
    ...
    block({
      ...
      fields: fields({ key: 'activas', label: 'Categorías activas', kind: 'number', default: 2, min: 0, max: 5 }),
      derived: [
        {
          resultKey: 'chips',
          name: 'chips',
          expr: 'slice categorias',
          compute: (v) => ['Ropa', 'Calzado', 'Accesorios', 'Ofertas', 'Nuevo'].slice(0, v.activas as number).join(' · ')
        }
      ]
    }),
    block({
...
export const day12: DayLabConfig = {
  ...
  labs: [
    ...
    block({
      ...
      fields: fields({ key: 'intentos', label: 'Intentos', kind: 'number', default: 2, min: 1, max: 5 }),
      derived: [
        {
          resultKey: 'ok',
          name: 'exito',
