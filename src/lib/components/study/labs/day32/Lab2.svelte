<script lang="ts">
  ...
  const stack = $derived(
    [values.svelte && 'SvelteKit', values.supabase && 'Supabase', values.vercel && 'Vercel'].filter(Boolean).join(' + ')
  );
  ...
  {#snippet preview()}
    <div class="flex flex-wrap gap-2">
      {#each [['svelte', 'SvelteKit'], ['supabase', 'Supabase'], ['vercel', 'Vercel']] as [k, label] (k)}
        <label class="flex items-center gap-2 rounded-full border px-4 py-2 {values[k as keyof typeof values] ? 'border-primary bg-primary/10' : ''}">
