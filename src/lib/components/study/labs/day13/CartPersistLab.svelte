	...
	{#snippet preview()}
		...
		<div class="mt-4 flex gap-2">
			{#each Array.from({ length: values.items }, (_, i) => i) as i (i)}
				<span class="size-12 rounded-lg bg-tertiary/20 flex items-center justify-center text-xs"
					>📦</span
				>
			{/each}
		</div>
		<pre
			class="mt-6 rounded-lg bg-surface-container-high p-4 font-mono text-sm overflow-x-auto">{json}</pre>
	{/snippet}
	{#snippet spec()}
		<LiveSpecPanel
			bind:values
			{results}
			limits={{ items: { min: 0, max: 5 } }}
			lines={[
				{ kind: 'let', key: 'items' },
