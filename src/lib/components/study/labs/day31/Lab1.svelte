<script lang="ts">
	...
	const step = $derived(
		[values.lint && 'lint ✓', values.test && 'test ✓', values.build && 'build ✓']
			.filter(Boolean)
			.join(' → ') || 'build falló'
	);
...
<InteractiveLabLayout hint="Pipeline: check → test → build antes de deploy.">
	{#snippet preview()}
		<ol class="space-y-2">
			{#each [{ key: 'lint', label: 'npm run check' }, { key: 'test', label: 'npm test' }, { key: 'build', label: 'npm run build' }] as s (s.key)}
				<li class="flex items-center gap-2 rounded-lg border px-4 py-2">
					<input
						type="checkbox"
						bind:checked={values[s.key as 'lint' | 'test' | 'build']}
						class="accent-primary"
					/>
					<span class={values[s.key as 'lint' | 'test' | 'build'] ? 'font-semibold' : 'opacity-40'}
						>{s.label}</span
					>
				</li>
			{/each}
