...
<InteractiveLabLayout hint="try/catch + retry — éxito tras suficientes intentos.">
	{#snippet preview()}
		...
		<p class="mt-4 font-bold {ok ? 'text-primary' : 'text-destructive'}">
			{ok ? 'Fetch OK' : 'Reintentando…'}
		</p>
	{/snippet}
	{#snippet spec()}
		<LiveSpecPanel
