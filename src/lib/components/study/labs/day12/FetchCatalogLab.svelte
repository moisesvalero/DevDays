...
<InteractiveLabLayout hint="async fetch — catálogo cargado o error de API.">
  {#snippet preview()}
    <label class="flex gap-3 mb-4"><input type="checkbox" bind:checked={values.exito} class="accent-primary" /> Fetch OK</label>
    <div class="grid grid-cols-3 gap-3">
      {#if values.exito}
        ...
        {/each}
      {:else}
        <div class="col-span-3 rounded-lg bg-destructive/10 p-6 text-center text-destructive font-bold">Error 500</div>
      {/if}
    </div>
