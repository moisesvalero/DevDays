...
<InteractiveLabLayout hint="then encadenado — validar stock luego confirmar pedido.">
	{#snippet preview()}
		<div class="flex items-center justify-center gap-4">
			<div
				class="rounded-lg border px-4 py-3 text-sm {values.stock > 0
			...
			<span>→</span>
			<div
				class="rounded-lg border px-4 py-3 font-bold {paso === 'Confirmado'
		...
		<input
			...
			class="mt-6 w-full max-w-xs accent-primary"
		/>
	{/snippet}
	{#snippet spec()}
		<LiveSpecPanel
