...
<InteractiveLabLayout hint="for genera filas — la tabla crece según count.">
  {#snippet preview()}
    ...
    <div class="overflow-hidden rounded-xl border border-outline-variant/40">
      <table class="w-full text-sm">
        <thead class="bg-surface-container-high text-left text-xs uppercase"><tr><th class="px-4 py-2">#</th><th class="px-4 py-2">SKU</th></tr></thead>
        <tbody>
          {#each filas as sku, i (sku)}
            <tr class="border-t border-outline-variant/30 transition-all duration-300" style="animation-delay: {i * 50}ms">
  ...
  {/snippet}
  {#snippet spec()}
    <LiveSpecPanel bind:values {results} limits={{ count: { min: 1, max: 8 } }} lines={[
