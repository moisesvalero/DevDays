...
<InteractiveLabLayout hint="forEach / slice — activa categorías en la barra de chips.">
  {#snippet preview()}
    <label class="mb-4 block max-w-xs space-y-2">
      ...
      <input type="range" min="0" max="5" bind:value={values.activas} class="w-full accent-tertiary" />
    </label>
    <div class="flex flex-wrap gap-2">
      {#each cats as cat, i (cat)}
        <span
          class="rounded-full px-4 py-2 text-sm font-semibold transition-all duration-300
            {i < values.activas ? 'bg-tertiary text-on-tertiary scale-105' : 'bg-outline-variant/20 text-on-surface-variant opacity-40'}"
        >
          {cat}
        </span>
      {/each}
  ...
  {/snippet}
  {#snippet spec()}
    <LiveSpecPanel bind:values {results} limits={{ activas: { min: 0, max: 5 } }} lines={[
