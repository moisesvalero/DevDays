...
<InteractiveLabLayout
	hint="Baja el stock por debajo de 5 — el badge pasa a «reponer» como en tu if/else."
>
	{#snippet preview()}
		<div class="mx-auto max-w-md space-y-6">
			<div class="rounded-xl border border-outline-variant/40 bg-surface-container-low p-6">
				...
				<label class="mt-4 block space-y-2">
					<span class="text-sm text-on-surface-variant"
						>Unidades en stock: <LiveValue value={values.stock} /></span
					>
					<input
						type="range"
						min="0"
          ...
          {estado === 'reponer'
			...
			>
				<span
					class="material-symbols-outlined text-4xl {estado === 'reponer'
						? 'text-destructive'
						: 'text-primary'}"
				>
				...
				<div>
					...
					<p
						class="text-3xl font-black uppercase tracking-wide {estado === 'reponer'
							? 'text-destructive'
							: 'text-primary'}"
					>
						{estado}
