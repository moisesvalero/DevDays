...
<InteractiveLabLayout hint="Parámetro por defecto en descuento — precioFinal recalcula al cambiar dto.">
  {#snippet preview()}
    <div class="grid gap-6 sm:grid-cols-2">
      <label class="rounded-xl border border-outline-variant/40 bg-surface-container-low p-5 space-y-2">
        <span class="text-sm text-on-surface-variant">Precio lista (€)</span>
        <input type="range" min="10" max="500" bind:value={values.precio} class="w-full accent-primary" />
      ...
      <label class="rounded-xl border border-outline-variant/40 bg-surface-container-low p-5 space-y-2">
        ...
        <span class="font-mono text-2xl font-bold text-tertiary"><LiveValue value={values.dto} />%</span>
      </label>
    </div>
