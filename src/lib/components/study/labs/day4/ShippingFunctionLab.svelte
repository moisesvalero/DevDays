...
<InteractiveLabLayout hint="function calcularEnvio — suma base + suplemento visible en el ticket.">
  {#snippet preview()}
    <div class="mx-auto max-w-sm rounded-2xl border border-outline-variant/50 bg-card p-6 shadow-md">
      ...
      <div class="mt-4 space-y-2 text-sm">
        <div class="flex justify-between">
          ...
          <input type="number" min="0" max="20" bind:value={values.base} class="w-16 rounded border px-2 font-mono text-right" />
        </div>
        <div class="flex justify-between">
          <span class="text-on-surface-variant">Urgente</span>
          <input type="number" min="0" max="15" bind:value={values.urgente} class="w-16 rounded border px-2 font-mono text-right" />
        </div>
