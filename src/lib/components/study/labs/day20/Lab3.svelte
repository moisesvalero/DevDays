<script lang="ts">
  ...
  const msg = $derived(values.news ? '¡Gracias por suscribirte!' : 'Marca la casilla para newsletter');
  const results = $derived({ msg });
</script>

<InteractiveLabLayout hint="bind:checked para checkboxes y radios.">
  {#snippet preview()}
    <label class="flex cursor-pointer items-start gap-4 rounded-xl border p-6 transition-colors hover:bg-surface-container-low">
