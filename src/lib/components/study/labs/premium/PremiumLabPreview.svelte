...
{#if variant === 'inventory'}
	<div class="grid gap-6 lg:grid-cols-2">
				...
				{#if field.kind === 'number'}
					<label class="mt-4 block space-y-2">
						...
						<span class="font-mono text-2xl font-bold text-on-surface"
							><LiveValue value={num(field.key)} /></span
						>
					</label>
				{/if}
			{/each}
		...
		<div
			class="flex flex-col items-center justify-center rounded-xl border-2 border-primary/30 bg-primary/5 p-8"
		>
			<p class="text-sm text-on-surface-variant">Resultado en UI</p>
			<p class="mt-2 font-mono text-5xl font-black text-primary">
				<LiveValue value={primaryResult} />
			</p>
		</div>
	</div>
{:else if variant === 'profile'}
	<div class="mx-auto max-w-md rounded-2xl border border-outline-variant/50 bg-card p-6 shadow-lg">
		<div class="flex items-center gap-4">
			<div
				class="flex size-16 items-center justify-center rounded-full bg-primary/15 text-2xl font-bold text-primary"
			>
				{str('nombre').charAt(0) || '?'}
			</div>
					...
					{#if field.kind === 'string'}
						<label class="block">
							...
							<input
								type="text"
								bind:value={values[field.key]}
								class="mt-1 w-full rounded-lg border border-outline-variant bg-surface-container-low px-3 py-2"
							/>
						</label>
		...
		<p
			class="mt-6 rounded-lg bg-primary/10 px-4 py-3 text-center text-sm font-medium text-on-surface"
		>
			{primaryResult}
		</p>
	</div>
			...
			<label class="space-y-2">
				...
				{#if field.kind === 'string'}
					<input
						type="text"
						bind:value={values[field.key]}
						class="rounded-lg border border-outline-variant px-3 py-2 font-medium"
					/>
				{:else if field.kind === 'number'}
					<input
						type="number"
						...
						max={field.max}
						bind:value={values[field.key]}
						class="w-28 rounded-lg border border-outline-variant px-3 py-2 font-mono text-xl"
					/>
				{/if}
			</label>
		{/each}
		<div
			class="lab-card-lift rotate-[-2deg] rounded-lg border-2 border-dashed border-tertiary bg-tertiary-container/20 px-6 py-4"
		>
			...
			{#if field.kind === 'number'}
				<label class="block max-w-sm space-y-2">
					<span class="text-sm text-on-surface-variant"
						>{field.label}: <LiveValue value={num(field.key)} /></span
					>
					<input
						type="range"
						min={field.min}
...
{:else if variant === 'cta'}
	<div class="flex flex-col items-center gap-6 py-4">
		<label
			class="flex cursor-pointer items-center gap-3 rounded-xl border border-outline-variant/50 bg-surface-container-low px-5 py-4"
		>
			<input
				type="checkbox"
				checked={bool(config.fields[0]?.key ?? 'logueado')}
				onchange={() => {
...
{:else if variant === 'plans'}
	<div class="grid gap-4 sm:grid-cols-3">
		{#each [{ n: 1, name: 'Free' }, { n: 2, name: 'Pro' }, { n: 3, name: 'VIP' }] as plan (plan.n)}
			<button
				type="button"
          ...
          {num('plan') === plan.n
			...
			>
				...
				{#if num('plan') === plan.n}
					<span
						class="mt-2 inline-block rounded-full bg-primary px-2 py-0.5 text-[10px] font-bold text-on-primary"
					>
						Activo
					</span>
				{/if}
          ...
          {i === 0 && primaryResult === num(field.key)
			...
			>
				...
				<input
					...
					max={field.max}
					bind:value={values[field.key]}
					class="mt-2 w-full rounded-lg border border-outline-variant bg-card px-3 py-2 font-mono text-3xl font-bold"
				/>
			</article>
		{/each}
	</div>
	<p class="mt-4 text-center text-sm text-on-surface-variant">
		Destacado: <span class="font-mono font-bold text-primary"
			><LiveValue value={primaryResult} /></span
		>
	</p>
			...
			{#if field.kind === 'boolean'}
				<label class="flex items-center gap-3">
					<input
						...
						onchange={() => (values[field.key] = !bool(field.key))}
						class="size-5 accent-primary"
					/>
...
{:else if variant === 'formula'}
	<div
		class="rounded-2xl border border-primary/20 bg-gradient-to-br from-primary/5 to-card p-8 text-center"
	>
		<p class="text-sm text-on-surface-variant">{config.uiDescription}</p>
		<div class="mt-6 grid gap-4 sm:grid-cols-2">
			{#each config.fields as field (field.key)}
				<label
					class="rounded-lg border border-outline-variant/40 bg-surface-container-low p-4 text-left"
				>
					<span class="text-xs text-on-surface-variant">{field.label}</span>
					{#if field.kind === 'number'}
						<input
		...
		<p class="mt-8 font-mono text-4xl font-black text-primary">
			<LiveValue value={primaryResult} />
		</p>
	</div>
	...
	<div class="space-y-4">
		...
		<div class="flex flex-wrap gap-2">
			{#each String(primaryResult).split(/,\s*/) as chip (chip)}
				<span
					class="rounded-full bg-tertiary/15 px-4 py-2 font-mono text-sm font-semibold text-tertiary"
					>{chip}</span
				>
			{/each}
		</div>
		<p class="rounded-lg bg-surface-container-high px-4 py-3 font-mono text-sm text-on-surface">
			{primaryResult}
		</p>
	</div>
{:else if variant === 'timeline'}
	<div class="flex items-center justify-center gap-2 py-6">
		{#each ['idle', 'loading', 'ok', 'error'] as step (step)}
          ...
          {String(primaryResult).toLowerCase().includes(step) ||
				(step === 'ok' && primaryResult === true)
...
{:else if variant === 'route'}
	<div
		class="rounded-xl border border-outline-variant/40 bg-surface-container-low p-5 font-mono text-sm"
	>
		<p class="text-xs font-bold uppercase text-primary">App shell</p>
		<div class="mt-3 space-y-1 text-on-surface-variant">
			<p class={str('ruta') === '/' || str('path') === '/' ? 'text-primary font-semibold' : ''}>
				/ inicio
			</p>
			<p
				class={str('ruta').includes('producto') || Boolean(str('slug'))
		...
		{#each config.fields as field (field.key)}
			{#if field.kind === 'string'}
				<input
					type="text"
					bind:value={values[field.key]}
					class="mt-4 w-full rounded border border-outline-variant px-2 py-1"
...
{:else if variant === 'form'}
	<form
		class="mx-auto max-w-sm space-y-4 rounded-xl border border-outline-variant/50 bg-card p-6"
			...
			<label class="block space-y-1">
				...
				{#if field.kind === 'string'}
					<input
						type="text"
						bind:value={values[field.key]}
						class="w-full rounded-lg border border-outline-variant px-3 py-2"
					/>
				{:else if field.kind === 'number'}
					<input
						type="number"
...
{:else if variant === 'auth'}
	<div class="mx-auto max-w-xs rounded-2xl border border-outline-variant/50 bg-card p-6 shadow-md">
		<p class="text-center text-xs font-bold uppercase tracking-widest text-primary">
			Supabase Auth
		</p>
		{#each config.fields as field (field.key)}
		...
		{/each}
		<div class="mt-4 rounded-lg bg-primary/10 px-3 py-2 text-center text-sm font-medium">
			{primaryResult}
		</div>
	</div>
{:else if variant === 'story'}
	<div class="grid gap-4 md:grid-cols-3">
		{#each config.fields as field, i (field.key)}
			<div
				class="lab-card-lift rounded-xl border border-outline-variant/40 bg-surface-container-low p-4"
			>
				...
				{#if field.kind === 'string'}
					<input
						type="text"
						bind:value={values[field.key]}
						class="mt-2 w-full rounded border border-outline-variant px-2 py-1 text-sm"
					/>
				{/if}
			</div>
		{/each}
		...
		{#each config.fields as field (field.key)}
			{#if field.kind === 'string'}
				<input
					type="text"
					bind:value={values[field.key]}
					class="mt-4 w-full rounded border border-outline-variant px-2 py-1 text-sm"
				/>
			{/if}
		{/each}
