...
<InteractiveLabLayout hint="$derived recalcula impuesto y total sin asignar variables manualmente.">
      ...
      <table class="w-full text-left text-sm">
        ...
        <tbody>
          <tr class="border-t">
            <td class="px-4 py-3">
              Base
              <input type="range" min="0" max="500" bind:value={values.base} class="mt-1 block w-full accent-primary" />
            </td>
            <td class="px-4 py-3 text-right font-mono font-bold"><LiveValue value={values.base} /></td>
          </tr>
          <tr class="border-t">
            <td class="px-4 py-3">
              IVA (<input type="range" min="0" max="30" bind:value={values.iva} class="inline w-20 align-middle accent-primary" />%)
            </td>
          ...
          <tr class="border-t bg-primary/10 font-bold">
            ...
            <td class="px-4 py-3 text-right font-mono text-2xl text-primary"><LiveValue value={total} /></td>
          </tr>
        </tbody>
      </table>
    </div>
  {/snippet}
  {#snippet spec()}
    <LiveSpecPanel bind:values {results} limits={{ base: { min: 0, max: 500 }, iva: { min: 0, max: 30 } }} lines={[
