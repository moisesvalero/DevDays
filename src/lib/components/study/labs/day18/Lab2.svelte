...
<InteractiveLabLayout hint="$derived para alertas: la UI cambia de color según stock vs umbral.">
  {#snippet preview()}
    <div class="grid gap-6 lg:grid-cols-2">
      <label class="rounded-xl border p-6 space-y-3">
        ...
        <input type="range" min="1" max="15" bind:value={values.umbral} class="w-full accent-tertiary" />
      </label>
      <div
  ...
  {/snippet}
  {#snippet spec()}
    <LiveSpecPanel bind:values {results} limits={{ stock: { min: 0, max: 20 }, umbral: { min: 1, max: 15 } }} lines={[
