...
<InteractiveLabLayout hint="interface Producto con nombre string y precio number.">
  {#snippet preview()}
    <div class="rounded-xl border p-6 font-mono text-sm">
      ...
      <input type="number" class="mt-2 w-full rounded border px-2 py-1" bind:value={values.precio} />
  ...
  {/snippet}
  {#snippet spec()}
    <LiveSpecPanel bind:values {results} limits={{ precio: { min: 0, max: 300 } }} lines={[
