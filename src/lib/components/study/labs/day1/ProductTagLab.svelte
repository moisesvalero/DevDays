...
<InteractiveLabLayout
	hint="Combina producto y precio — la etiqueta de checkout es tu template literal."
>
	{#snippet preview()}
		<div class="flex flex-col items-center gap-8 py-4 lg:flex-row lg:justify-center">
			<div
				class="w-full max-w-xs space-y-4 rounded-xl border border-outline-variant/40 bg-surface-container-low p-5"
			>
				...
				<label class="block space-y-1">
					...
					/>
					<span class="font-mono text-xl font-bold text-tertiary"
						><LiveValue value={values.precio} /> €</span
					>
				</label>
			</div>

			<div
				class="lab-card-lift relative rotate-[-3deg] rounded-xl border-2 border-tertiary bg-tertiary-container/30 px-8 py-6 shadow-lg"
			>
				<span
					class="absolute -right-2 -top-2 rounded-full bg-tertiary px-2 py-0.5 text-[10px] font-bold text-on-tertiary"
				>
					CHECKOUT
				</span>
