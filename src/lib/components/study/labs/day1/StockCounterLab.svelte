...
<InteractiveLabLayout hint="Cambia `cajas` o `entrada` en el spec — el almacén recalcula el total al instante.">
  {#snippet preview()}
    <div class="grid gap-6 lg:grid-cols-2">
      <div class="rounded-xl border border-outline-variant/40 bg-surface-container-low p-6">
        ...
        <p class="mt-1 text-sm text-on-surface-variant">Simula <code class="text-primary">let cajas</code> y un ingreso de mercancía.</p>

        <div class="mt-6 flex items-end gap-4">
          <div class="text-center">
            <p class="text-[10px] uppercase text-on-surface-variant">Stock actual</p>
            <p class="font-mono text-4xl font-black text-on-surface"><LiveValue value={values.cajas} /></p>
          </div>
          <span class="pb-2 text-2xl text-primary">+</span>
          <div class="text-center">
            <p class="text-[10px] uppercase text-on-surface-variant">Entrada</p>
            <p class="font-mono text-4xl font-black text-primary"><LiveValue value={values.entrada} /></p>
          </div>
        </div>

        <label class="mt-6 block space-y-2">
          <span class="text-sm text-on-surface-variant">Unidades en stock</span>
          <input type="range" min="0" max="50" bind:value={values.cajas} class="w-full accent-primary" />
        </label>
        <label class="mt-4 block space-y-2">
          <span class="text-sm text-on-surface-variant">Unidades a sumar</span>
          <input type="range" min="1" max="20" bind:value={values.entrada} class="w-full accent-primary" />
        </label>
      </div>
      ...
      >
        <p class="text-sm font-medium text-on-surface-variant">Después de <code>cajas += entrada</code></p>
        <p class="mt-4 font-mono text-6xl font-black text-primary"><LiveValue value={stockFinal} /></p>
