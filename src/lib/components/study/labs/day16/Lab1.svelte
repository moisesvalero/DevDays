...
<InteractiveLabLayout hint="&lt;script&gt; define lógica; el markup usa las variables.">
	{#snippet preview()}
		<div class="grid gap-6 lg:grid-cols-2 font-mono text-sm">
			...
			<div
				class="flex flex-col items-center justify-center rounded-xl border-2 border-dashed border-primary/40 p-8"
			>
				<p class="text-5xl font-black text-primary"><LiveValue value={values.count} /></p>
				<button
					type="button"
					class="mt-4 rounded-lg bg-primary px-6 py-2 font-bold text-on-primary"
					onclick={inc}
				>
					count++
				</button>
