...
<InteractiveLabLayout hint="Closure privado — cada clic incrementa el contador encapsulado.">
  {#snippet preview()}
    <div class="text-center">
      ...
      <input type="range" min="0" max="10" bind:value={values.clics} class="mt-4 w-full max-w-xs accent-primary" />
    </div>
  {/snippet}
  {#snippet spec()}
    <LiveSpecPanel bind:values {results} limits={{ clics: { min: 0, max: 10 } }} lines={[
