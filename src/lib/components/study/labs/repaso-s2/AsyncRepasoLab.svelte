<script lang="ts">
  ...
  const estado = $derived(values.loading ? 'Cargando…' : values.ok ? 'Catálogo cargado' : 'Error red');
        ...
        {#each ['idle', 'loading', 'done', 'error'] as step, i (step)}
          <div
            class="h-2 flex-1 rounded-full transition-colors
              {(values.loading && i === 1) || (!values.loading && values.ok && i <= 2) || (!values.ok && !values.loading && i === 3)
              ? 'bg-primary'
