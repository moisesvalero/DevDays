  ...
  {#snippet preview()}
    <div class="space-y-6">
      <label class="block max-w-xs space-y-2">
        ...
        <input type="range" min="1" max="5" bind:value={values.factor} class="w-full accent-primary" />
        <span class="font-mono font-bold">× <LiveValue value={values.factor} /></span>
      </label>
      <div class="flex flex-wrap gap-3">
        {#each base as precio, i (i)}
          <div class="rounded-xl border border-outline-variant/40 bg-surface-container-low px-4 py-3 text-center">
