  ...
  {#snippet preview()}
    <div class="grid gap-4 sm:grid-cols-3">
      <div class="rounded-xl bg-surface-container-low p-4 text-center">
        ...
        <input type="range" min="0" max="500" bind:value={values.base} class="mt-2 w-full accent-primary" />
        <p class="font-mono text-2xl font-bold"><LiveValue value={values.base} />€</p>
      </div>
      <div class="rounded-xl bg-surface-container-low p-4 text-center">
        <p class="text-xs uppercase">IVA</p>
        <input type="range" min="0" max="30" bind:value={values.iva} class="mt-2 w-full accent-primary" />
