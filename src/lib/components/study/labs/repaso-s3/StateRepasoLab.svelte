...
<InteractiveLabLayout hint="$state: cantidad mutable en el carrito.">
	{#snippet preview()}
		<div class="flex items-center gap-6 rounded-xl border border-primary/20 bg-primary/5 p-8">
			<button
				type="button"
				class="size-12 rounded-full bg-card text-2xl font-bold shadow"
			...
			<button
				type="button"
				class="size-12 rounded-full bg-primary text-2xl font-bold text-on-primary shadow"
				onclick={mas}>+</button
			>
		</div>
	{/snippet}
