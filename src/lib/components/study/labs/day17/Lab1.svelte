...
<InteractiveLabLayout hint="$state guarda cantidad y precio; la UI reacciona al instante.">
		...
		<div class="grid gap-6 lg:grid-cols-2">
			<div class="rounded-xl border border-outline-variant/40 bg-surface-container-low p-6">
				...
				<div class="mt-6 flex items-center justify-center gap-4">
					<button
						type="button"
						class="size-10 rounded-full bg-card text-xl font-bold shadow"
						onclick={menos}>−</button
					>
					<span class="font-mono text-5xl font-black"><LiveValue value={values.qty} /></span>
					<button
						type="button"
						class="size-10 rounded-full bg-primary text-xl font-bold text-on-primary"
						onclick={mas}>+</button
					>
				</div>
			...
			<div
				class="flex flex-col justify-center rounded-xl border-2 border-dashed border-primary/40 bg-primary/5 p-8 text-center"
			>
	...
	{#snippet spec()}
		<LiveSpecPanel
			bind:values
			{results}
			limits={{ qty: { min: 0, max: 10 }, precio: { min: 5, max: 80 } }}
			lines={[
