      ...
      {#each [['svelte', 'SvelteKit'], ['supabase', 'Supabase'], ['vercel', 'Vercel']] as [key, label] (key)}
        <label
          class="flex cursor-pointer items-center gap-2 rounded-full border px-4 py-2 text-sm font-medium transition-colors
            {values[key as keyof typeof values] ? 'border-primary bg-primary/10 text-primary' : 'opacity-60'}"
        >
          <input type="checkbox" bind:checked={values[key as 'svelte' | 'supabase' | 'vercel']} class="accent-primary" />
          {label}
        </label>
      {/each}
    </div>
