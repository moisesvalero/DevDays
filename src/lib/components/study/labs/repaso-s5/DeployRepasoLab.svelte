<script lang="ts">
  ...
  const url = $derived(values.live ? `https://tu-app.vercel.app · ${steps[values.step]}` : 'Solo local');
...
<InteractiveLabLayout hint="Del commit al deploy en producción.">
      ...
      <div class="flex justify-between gap-1">
        {#each steps as s, i (s)}
          <div class="flex flex-1 flex-col items-center gap-1">
            <div
              class="h-2 w-full rounded-full {i <= values.step ? 'bg-primary' : 'bg-outline-variant/30'}"
            ></div>
  ...
  {/snippet}
  {#snippet spec()}
    <LiveSpecPanel bind:values {results} limits={{ step: { min: 0, max: 3 } }} lines={[
